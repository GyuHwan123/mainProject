...
def _normalize_expense_category(value: Any, evidence_text: Any = None) -> str:
    ...
    normalized = by_compact.get(compact, "기타")
...
def _normalize(result: dict[str, Any], filename: str, text: str) -> dict[str, Any]:
    ...
    normalized_record = {
        ...
        "expense_category": _normalize_expense_category(
            hints.get("expense_category") or result.get("expense_category"),
            text,
        ),
...
2. expense_category는 상호·품목·문맥으로 고르되 불명확하면 `기타`입니다. `식비/주류`는 소주·맥주·와인·위스키 등 명확한 주류 근거가 있을 때만 선택하고, 그 외 음식·과자·일반 음료는 `식비`입니다.
...
def _item_parser_profile(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    relationship_rules = {
        ...
        "parent-child": [
            ...
            "Store a supported child as specification or note; emit it separately only with evidence that it was independently sold.",
        ],
    }[relationship]
    return {
