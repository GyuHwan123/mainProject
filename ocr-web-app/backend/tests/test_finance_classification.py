...
class FinanceClassificationTests(unittest.IsolatedAsyncioTestCase):
    def test_simple_v1_prompt_requests_all_fields_once(self):
        ...
        self.assertEqual(FINANCE_PROMPT_VERSION, "receipt-simple-v1.7-category-only")
        self.assertNotIn("expense_category_suggestion", prompt)
        self.assertNotIn("expense_category_evidence", prompt)
    ...
    def test_normalization_does_not_repair_wrong_amount(self):
        ...
        # User confirmation is the workflow state, even when extraction needs review.
        self.assertEqual(validation["decision"], "USER_CONFIRM")
        extraction = normalized["structured_data"]["extraction_validation"]
        self.assertEqual(extraction["decision"], "REVIEW")
        self.assertIn("TOTAL_AMOUNT_NOT_IN_OCR", extraction["reasons"])
