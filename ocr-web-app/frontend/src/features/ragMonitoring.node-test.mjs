...
const Chart = new Function('React', chartCode + '\nreturn RagMonitoringChart;')(React);
const page = fs.readFileSync(new URL('../pages/ReportPage.jsx', import.meta.url), 'utf8');
const reportCode = (await transformWithOxc(page.slice(page.indexOf('function RagPerformanceReport('), page.indexOf('\nfunction RagLlmEvaluation(')), 'Report.jsx', { jsx: { runtime: 'classic' } })).code;
const factory = new Function('React', 'useMemo', 'useState', 'useEffect', 'apiClient', 'RagMonitoringChart', 'createInitialMonitoringDateRange', 'dateInputValue', 'percent', 'IoDownloadOutline', reportCode + '\nreturn RagPerformanceReport;');

function harness({ data = null, api = async () => ({ data }), range = { startDate: '2026-09-01', endDate: '2026-09-07' }, refresh = 0 } = {}) {
  const states = [range, '7', data, false, ''];
  const changes = [];
  const effects = [];
  const requests = [];
  let cursor = 0;
  ...
  }, (effect, deps) => effects.push({ effect, deps }), { get: (...args) => { requests.push(args); return api(...args); } }, Chart, () => range, d => d.toISOString().slice(0, 10), v => `${((v || 0) * 100).toFixed(1)}%`, () => null);
...
test('selected dates and refresh are wired to the read-only monitoring API', async () => {
  ...
  assert.deepEqual(h.effects[0].deps, ['2026-09-01', '2026-09-07', 3]);
});
