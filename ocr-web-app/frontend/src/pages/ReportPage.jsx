...
import RagMonitoringChart, { RAG_TREND_METRICS } from '../components/RagMonitoringChart';
...
function RagResponseDistribution({ demoMode, monitoring, loading }) {
  ...
  const colors = [RAG_KPI_COLORS[1], RAG_KPI_COLORS[5], RAG_KPI_COLORS[2], '#8a98aa'];
  let offset = 0;
  const rows = counts.map((count, index) => ({ count, label: labels[index], color: colors[index], rate: count / total })).filter(row => row.count > 0);
  const gradient = `conic-gradient(${rows.map(row => {
    const start = offset; offset += row.rate * 100;
...
function RagPerformanceReport({ evaluation: latestEvaluation, modelConfig, umapData, umapError, onExportPdf, refreshVersion, demoMode = false }) {
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [dateRange.startDate, dateRange.endDate, refreshVersion, demoMode]);
  ...
  return <section className="receipt-monitoring rag-monitoring">
    ...
    <div className="receipt-monitoring-kpis" aria-busy={monitoringLoading}>{kpis.map(([label, value, format], index) => <article key={label}>
      <h3>{label}</h3>
      <div className="monitoring-kpi-value"><strong style={{ color: value == null ? undefined : RAG_KPI_COLORS[index] }}>{monitoringLoading ? '…' : format === 'cases' ? (value ?? '—') : metricValue(value)}</strong><small>선택 기간 · {format === 'cases' ? '누적 문항' : '실행별 평균'}</small></div>
      <RagMonitoringChart daily={monitoring?.daily || []} metric={['total', 'answer_accuracy', 'faithfulness', 'hit_at_1', 'context_precision', 'hallucination_rate'][index]} color={RAG_KPI_COLORS[index]} />
    </article>)}</div>
    ...
    {!evaluation && !demoMode && <p className="rag-report-empty">전체 문항이 완료되어 DB에 저장된 실행만 모니터링에 표시됩니다.</p>}
  </section>;
}
