...
export default function ResetPasswordPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setError(requestError?.response?.data?.detail || '비밀번호를 변경하지 못했습니다. 링크를 다시 확인해 주세요.');
  ...
  return <div className="login-shell"><div className="login-window reset-password-window"><div className="login-panel">
    ...
    <form className="login-form" onSubmit={submit} noValidate>
      ...
      {!success && <>
        <label><span>새 비밀번호</span><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="new-password" minLength={8} disabled={loading} /></label>
        <label><span>새 비밀번호 확인</span><input type="password" value={confirmation} onChange={(e) => setConfirmation(e.target.value)} autoComplete="new-password" minLength={8} disabled={loading} /></label>
