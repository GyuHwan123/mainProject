...
def load_inputs(label: str) -> tuple[list[dict], np.ndarray, dict]:
    with Path("/input/company_chunks.pkl").open("rb") as file:
        chunks = pickle.load(file)
    with Path(f"/output/{label}_embeddings.pkl").open("rb") as file:
        embeddings = np.asarray(pickle.load(file))
    metadata = json.loads(Path(f"/output/{label}_metadata.json").read_text(encoding="utf-8"))
    ...
    if metadata.get("label") != label:
        raise ValueError("Embedding metadata label이 일치하지 않습니다.")
    if metadata.get("chunk_fingerprint_sha256") != EXPECTED_FINGERPRINT:
        raise ValueError("Embedding chunk fingerprint가 일치하지 않습니다.")
    if chunk_fingerprint(chunks) != EXPECTED_FINGERPRINT:
        raise ValueError("입력 chunk fingerprint가 일치하지 않습니다.")
...
def context(client: httpx.Client, base: str, headers: dict[str, str]) -> dict:
    ...
    other_documents = get_all(client, base + "/rag_documents", headers, {
        "select": "*", "doc_id": f"not.in.({','.join(DOC_IDS)})", "order": "id.asc",
    })
    ...
    return {
        ...
        "other_documents_hash": stable_hash(other_documents),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="기업 RAG chunk의 embedding 필드만 동기화합니다.")
    parser.add_argument("--label", default="baseline", help="embedding variant label")
    parser.add_argument("--apply", action="store_true", help="embedding-only PATCH 수행")
    args = parser.parse_args()
    if not args.label.replace("-", "").replace("_", "").isalnum():
        parser.error("label에는 영문, 숫자, '-'와 '_'만 사용할 수 있습니다.")
    url = os.environ.get("SUPABASE_URL", "").strip()
    key = os.environ.get("SUPABASE_SERVICE_ROLE_KEY", "").strip()
    if not url or not key:
        raise RuntimeError("SUPABASE_URL과 SUPABASE_SERVICE_ROLE_KEY가 필요합니다.")
    chunks, embeddings, metadata = load_inputs(args.label)
    ...
    with httpx.Client(timeout=120) as client:
        ...
        if len(before) != EXPECTED_CHUNKS:
            raise RuntimeError(f"기업 chunk가 {EXPECTED_CHUNKS}개가 아닙니다: {len(before)}")
        indexed_before = {(row["document_id"], int(row["chunk_index"])): row for row in before}
        immutable_before = stable_hash([{k: v for k, v in row.items() if k != "embedding"} for row in before])
        for chunk in chunks:
            lookup = (state["document_ids"][chunk["doc_id"]], int(chunk["chunk_index"]))
            row = indexed_before.get(lookup)
            if not row or row["content"] != chunk["text"] or int(row["page_number"]) != int(chunk["page"]):
                raise RuntimeError("기업 chunk document_id/chunk_index/content 사전 검증에 실패했습니다.")
        preflight = {
            "company_documents": len(state["documents"]),
            "company_chunks_before": len(before),
            "other_documents_before": len(state["other_documents"]),
            "other_chunks_before": len(state["other_chunks"]),
            "other_documents_hash_before": state["other_documents_hash"],
            "other_chunks_hash_before": state["other_hash"],
            **metadata,
        }
        if not args.apply:
        ...
        try:
            for chunk, embedding in zip(chunks, embeddings):
                ...
                response = client.patch(
                    ...
                    headers=request_headers(key, "return=representation"),
                    json={"embedding": embedding.astype(np.float32).tolist()},
                )
                ensure_ok(response, f"{chunk['doc_id']}:{chunk_index} embedding PATCH")
                if len(response.json()) != 1:
                    raise RuntimeError("embedding PATCH 대상 행 수가 1개가 아닙니다.")
                patched.append((document_id, chunk_index))
        except Exception:
            for document_id, chunk_index in patched:
                old = indexed_before[(document_id, chunk_index)]
                client.patch(
                    base + "/rag_chunks",
                    params={"document_id": f"eq.{document_id}", "chunk_index": f"eq.{chunk_index}"},
                    headers=request_headers(key, "return=minimal"),
                    json={"embedding": parse_vector(old["embedding"]).tolist()},
                )
            raise

        final_company = company_rows(client, base, headers, ids)
        final_state = context(client, base, headers)
        indexed_final = {(row["document_id"], int(row["chunk_index"])): row for row in final_company}
        dimensions = []
        exact_embeddings = 0
        fingerprint_rows = []
        for chunk, expected_embedding in zip(chunks, embeddings):
            document_id = final_state["document_ids"][chunk["doc_id"]]
            chunk_index = int(chunk["chunk_index"])
            row = indexed_final[(document_id, chunk_index)]
            actual_embedding = parse_vector(row["embedding"])
            dimensions.append(actual_embedding.shape)
            exact_embeddings += int(np.allclose(actual_embedding, expected_embedding, rtol=0, atol=1e-6))
            fingerprint_rows.append({
                "doc_id": chunk["doc_id"], "chunk_index": chunk_index,
                "page": int(row["page_number"]), "text": row["content"],
            })
        immutable_final = stable_hash([{k: v for k, v in row.items() if k != "embedding"} for row in final_company])
        result = {
            "company_documents": len(final_state["documents"]),
            "company_chunks": len(final_company),
            "other_documents": len(final_state["other_documents"]),
            "other_chunks": len(final_state["other_chunks"]),
            "embedding_1024_count": sum(shape == (EXPECTED_DIMENSIONS,) for shape in dimensions),
            "embedding_exact_match_count": exact_embeddings,
            "content_fingerprint": chunk_fingerprint(fingerprint_rows),
            "company_non_embedding_fields_unchanged": immutable_final == immutable_before,
            ...
            "rag_documents_unchanged": stable_hash(final_state["documents"]) == state["documents_hash"],
            "other_documents_unchanged": (
                len(final_state["other_documents"]) == len(state["other_documents"])
                and final_state["other_documents_hash"] == state["other_documents_hash"]
            ),
        ...
        valid = (
            ...
            and result["embedding_exact_match_count"] == EXPECTED_CHUNKS
            and result["content_fingerprint"] == EXPECTED_FINGERPRINT
            and result["company_non_embedding_fields_unchanged"]
            and result["table_chunk_count"] > 0 and result["lodging_row_count"] == 1
            and result["flattened_lodging_count"] == 0
            and result["rag_documents_unchanged"]
            and result["other_documents_unchanged"] and result["other_chunks_unchanged"]
        )
        if not valid:
